      ...
      <div className="w-32 hidden sm:flex items-center gap-2">
        {onToggleLeft && (
          <button
            ...
            className="hidden md:inline-flex items-center gap-2 text-xs text-gray-300 hover:text-white bg-gray-800/70 border border-gray-700 rounded-full px-3 py-1.5 transition"
            aria-label="Toggle left sidebar"
          >
            <PanelLeft size={14} />
            {isLeftSidebarOpen ? 'Hide' : 'Show'} Panel
          </button>
        )}
      </div>
      ...
      <div className="flex flex-col items-center flex-1 max-w-md mx-auto px-2">
        ...
        <div className="relative w-full">
          <select
            ...
            className="w-full bg-gray-800 border border-gray-600 text-white text-sm rounded-full px-4 py-2 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 hover:bg-gray-700 cursor-pointer transition-colors disabled:opacity-50 disabled:cursor-not-allowed appearance-none text-center"
          >
            {styles.map(s => (
