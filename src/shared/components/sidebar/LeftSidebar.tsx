  ...
  return (
    <SidebarContainer isOpen={isOpen} width="w-full sm:w-80" side="left" onClose={onClose}>
      ...
      <div className="p-4 border-b border-gray-800 flex items-start justify-between gap-2 flex-shrink-0 bg-gray-900">
        <div>
          <h1 className={uiClass(UI_TYPOGRAPHY.heading, 'bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent')}>
            MapAlchemist
          </h1>
          <p className={uiClass(UI_TYPOGRAPHY.compact, 'text-gray-500 mt-1')}>AI Map Style Generator</p>
        </div>
        {onClose && (
          <button
            type="button"
          ...
          return (
            <div key={section.id} className="space-y-1">
              ...
              <div
                ...
                className={`flex items-center gap-2 px-3 py-2 cursor-pointer select-none border-b ${section.tailwindBorderColor} bg-gray-900/50 hover:bg-gray-800/50 transition-colors sticky top-0 z-10 backdrop-blur-sm`}
              >
                ...
                <span className={uiClass(UI_TYPOGRAPHY.sectionLabel, section.tailwindTextColor)}>
                  {section.title}
                </span>
              </div>
