...
export class PopupGenerator {
    ...
    static derivePopupSections(feature: any, details?: PoiPopupDetails | null): PopupSections {
        ...
        return {
            ...
            categoryLabel: String(subcategory || category || ''),
            description: String(detailState.summary || props.description || props.address || subcategory || category || ''),
    ...
    static generateHtml(
        ...
        const loadingBlock = sections.showLoading
            ? `
                <div data-testid="poi-popup-loading" style="margin-top:8px; padding:10px 11px; font-size:11px; border:1px solid ${PopupGenerator.withAlpha(border, '55')}; border-radius:10px; background:${loadingShellBg}; color:${text}; overflow:hidden;">
                    <style>
                    ...
                    <div style="display:grid; gap:6px; margin-top:8px;">
                        <div data-testid="poi-popup-loading-line-primary" style="height:7px; width:72%; border-radius:999px; background:${loadingLineStrong}; animation:mapAlchemistPopupSkeletonPulse 1.6s ease-in-out infinite;"></div>
                        <div data-testid="poi-popup-loading-line-secondary" style="height:6px; width:54%; border-radius:999px; background:${loadingLine}; animation:mapAlchemistPopupSkeletonPulse 1.6s ease-in-out infinite 0.12s;"></div>
                    </div>
                </div>
            `
            : '';
        ...
        const photoBlock = sections.photoUrl
            ...
            `
            : '';
        const summaryBlock = sections.description
            ? `
        ...
        return `
        <div data-testid="poi-popup" data-mapalchemist-popup-radius="${frameRadius}" style="position:relative; font-family:${popupStyle.fontFamily}; min-width:260px; width:min(400px, calc(100vw - 40px)); max-width:400px; box-sizing:border-box; padding-bottom:${PopupGenerator.FRAME_ARROW_HEIGHT}px;">
            ...
            <div data-mapalchemist-popup-content="true" style="position:relative; z-index:1; color:${text}; padding:12px 12px 10px; width:100%; box-sizing:border-box; max-height:min(56vh, 388px); overflow-y:auto; overflow-x:hidden;">
                <div style="display:flex; gap:10px; align-items:center;">
                    <div style="width:60px; height:60px; flex:0 0 60px; background:rgba(0,0,0,0.05); border-radius:12px; padding:4px; display:flex; align-items:center; justify-content:center; box-shadow:inset 0 0 0 2px ${border}40;">
                    ...
                    <div style="flex:1; min-width:0; padding-right:12px;">
                        ...
                        <div data-testid="poi-popup-category" style="font-size:10px; text-transform:uppercase; font-weight:bold; opacity:0.7; overflow-wrap:anywhere;">${PopupGenerator.escapeHtml(sections.categoryLabel)}</div>
                    </div>
                </div>
                ${summaryBlock}
                ${photoBlock}
                ...
                ${actionLinks ? `<div data-testid="poi-popup-actions" style="display:grid; grid-template-columns:repeat(auto-fit, minmax(150px, 1fr)); gap:7px; margin-top:10px;">${actionLinks}</div>` : ''}
