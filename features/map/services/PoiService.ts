...
export class PoiService {
            ...
            if (layer.id && layer.id !== 'unclustered-point') {
                try {
                    if (rawMap.getLayer(layer.id)) {
                        ...
                        console.log(`[PoiService] Hidden base map POI layer: ${layer.id}`);
                    }
                } catch (e) {
                    console.warn(`[PoiService] Failed to hide layer ${layer.id}:`, e);
                }
            }
        });
    }
    ...
    static refreshData(
        ...
        console.log(`[PoiService] Found ${features.length} POI features from ${poiSources.length} sources`);
        ...
        console.log(`[PoiService] Updated places source with ${features.length} features`);
    }
}
